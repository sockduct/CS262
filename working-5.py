...
# start - string, starting value for rules
# tokens - List, end result from start
def isambig(grammar,start,tokens):
    allgrmpaths = expand(grammar)
    if len([path[1] for path in allgrmpaths if start == path[0] and tokens == path[2]]) > 1:
        return True
    else:
        return False
...
def expand(grammar,upath=None):
    ...
    for i in range(len(grammar)):
        ...
        count = 0
        upath['morepaths'] = 1
        while upath['morepaths'] >= 1 and count < 10:
            upath['seedrule'] = i
            upath['currentrule'] = i
            upath['currentresult'] = []
            upath['currentpath'] = [i]
            ...
            if upath['morepathflag'] and upath['morepaths'] < 1:
                upath['morepaths'] += 1
                #print "More path flag set, incrementing morepaths (" + \
                #    str(upath['morepaths']) + ")"
        # End of while
        ...
        if upath['morepaths'] >= 1:  # Non-terminal
            for lhs in lhsmatches(grammar,rhs):
                ...
                if upath['currentpath']+[lhs[2]] not in upath['visited']:
                    ...
                    # If still more paths, set flag to return later
                    if upath['morepaths'] >= 1:
                        upath['morepathflag'] = True
